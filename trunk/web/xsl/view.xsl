...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="siblings/div">
    ...
    <xsl:variable name="linktext">
      <xsl:choose>
        ...
        <xsl:when test="starts-with(head, 'Bibliography')">Bibliography</xsl:when>
        <xsl:when test="starts-with(head, 'Index')">Index</xsl:when>
        <xsl:otherwise>
  ...
  <xsl:template match="siblings//head">
    <xsl:apply-templates/>
  </xsl:template>
